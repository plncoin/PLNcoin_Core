...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || IsArgSet("-?") || IsArgSet("-h") || IsArgSet("-help"))
    {
        ...
        std::string strUsage = strprintf(_("%s plncoin-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  plncoin-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded.plncoin transaction") + "\n" +
              "  plncoin-tx [options] -create [commands]   " + _("Create hex-encoded.plncoin transaction") + "\n" +
              "\n";
